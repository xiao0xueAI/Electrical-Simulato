...
const UI = {
  ...
  setRecBg(color) {
    ...
    // Persistent toolbar indicator: swatch + 白/黑 label (always visible after dropdown closes)
    const label = document.getElementById('recBgLabel');
    const swatch = document.getElementById('recBgSwatch');
    if (label) label.textContent = isWhite ? '白' : '黑';
    if (swatch) swatch.style.background = isWhite ? '#ffffff' : '#0d1117';
    // Checkmark inside dropdown menu items
    document.querySelectorAll('#recBgMenu .bg-check').forEach(c => c.style.display = 'none');
    const selCheck = document.querySelector('#recBgMenu .bg-sel[data-bg="' + color + '"] .bg-check');
    if (selCheck) selCheck.style.display = 'inline';
