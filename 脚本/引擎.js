...
const Engine = {
  ...
  solve() {
    ...
    // ==================== E2: 直流电源极性/反接告警 ====================
    // 检查任意两直流电源之间的端子连接关系，识别反向并联（短路）与反向串联（电压抵消）
    const dcBatts = batteries.filter(b => getPowerType(b) === 'dc');
    const nodesConnected = (aKey, bKey) => {
      ...
      while (stack.length) {
        ...
        for (const n of (graph.get(cur) || [])) {
          if (n === bKey) return true;
          if (!seen.has(n)) { seen.add(n); stack.push(n); }
        }
      }
      return false;
    };
    for (let i = 0; i < dcBatts.length; i++) {
      for (let j = i + 1; j < dcBatts.length; j++) {
        const A = dcBatts[i], B = dcBatts[j];
        const aPos = mkKey(A.id, 'p'), aNeg = mkKey(A.id, 'n');
        const bPos = mkKey(B.id, 'p'), bNeg = mkKey(B.id, 'n');
        const ap_bp = nodesConnected(aPos, bPos);
        const ap_bn = nodesConnected(aPos, bNeg);
        const an_bp = nodesConnected(aNeg, bPos);
        const an_bn = nodesConnected(aNeg, bNeg);
        // 反向并联：A+↔B− 且 A−↔B+ → 两电源互怼，必短路/大电流
        if (ap_bn && an_bp) {
          faults.push({ type: 'short', msg: `直流电源反接：「${A.name}」与「${B.name}」反向并联（会产生短路/大电流）`, comp: '' });
        }
        // 反向串联：A+ 接到 B− 且 A− 未接 B → 极性相反串联，电压相互抵消
        else if (ap_bn && !an_bp && !an_bn) {
          faults.push({ type: 'warn', msg: `直流电源反接串联：「${A.name}」与「${B.name}」极性相反串联，电压相互抵消`, comp: '' });
        }
        // 正向串联：A+ 接到 B+ 且 A− 未接 B → 正常串联升压，不告警
      }
    }
    ...
    batteries.forEach(bat => {
      ...
      S.components.forEach(comp => {
        if (comp.type === 'dry_relay' || comp.type === 'bt_relay' || comp.type === 'relay5' || comp.type === 'relay8' || comp.type === 'contactor') {
          ...
          if (I_A > rated) {
            faults.push({ type: 'warn', msg: `继电器「${comp.name}」触点电流 ${I_A.toFixed(2)}A 超过额定 ${rated}A`, comp: comp.name });
          }
        }
      });
